...
const FormInput = ({ isRegister, toLogin, setShowLogo }) => {
  ...
  return (
    <View className="flex-col  justify-center items-center text-center">
      <View
        className={`flex gap-4 ${
          avoidKeyboard ? "-mt-[60px]" : "mt-[2px]"
        } w-[320px]`}
      >
        <TextInput
          onChangeText={(text) => {
        ...
        {!errForm.confirmpassword && (
          <ErrorValidate err={"Mật khẩu phải dài hơn 8 kí tự"}></ErrorValidate>
        )}
        <TextInput
          onChangeText={(text) => {
        ...
        ></TextInput>
        {!errForm.confirmpassword && (
          <ErrorValidate err={"Mật khẩu xác nhận không đúng"}></ErrorValidate>
        )}
      </View>

      {stateRegister ? (
        <View className="flex-row flex-wrap w-[93%] justify-center mt-8">
          <Text className="-mt-3 text-[14px] text-colorBrownDarkLV2 font-semibold">
            Tin nhắn đã được gửi đến email của bạn. Vui lòng xác nhận và quay
            lại <Text className="font-bold text-colorBrownBold">Đăng Nhập</Text>
          </Text>
        </View>
      ) : (
        ""
      ...
      {!isRegister && (
        <View className="flex-row mt-4">
          <View className="flex-row items-center ">
            ...
            <Text className="ml-1 text-colorBrownDarkLV2">
              Ghi nhớ đăng nhập
            </Text>
          </View>
          <View className="mt-1">
            <TouchableOpacity>
              <Text className=" text-colorBrownDarkLV2 font-bold ml-12">
                Quên mật khẩu ?
              </Text>
            </TouchableOpacity>
          </View>
