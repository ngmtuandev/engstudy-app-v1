import { View, Text, TouchableOpacity, ImageBackground, Image, Pressable, TextInput, Alert } from 'react-native'
    ...
    const handleAddVocabulary = async () => {
      try {
        ...
        if (+newVocabulary?.status === 0) {
          showAlert('Bạn đã thêm từ vựng thành công', 'Bạn có muốn tiếp tục thêm từ vựng không ?')
        }
        else {
          showAlert('Từ vựng này đã tồn tại', 'Vui lòng chọn từ vựng khác chưa có trong từ điển')
        }
      } catch (error) {
        console.log(error)
      }
            ...
            className=' w-screen h-screen'>
                <View className='flex-col flex-1'>
                  <View className='flex-col mt-20 justify-center items-center '>
                    <View>
                      <Text className='mb-2 text-colorBrownDarkLV2 font-bold text-[18px]'>Từ loại</Text>
                      <View className='w-[300px] flex-row items-center justify-around 
                      rounded-md grid-cols-4 h-[50px] bg-colorBrownSlightLV2'>
                        {
                          CATEGORY_VOCABULARY.map((item, index) => {
                            return <Pressable 
                                    ...
                                    h-[30px] ${category === item ? 'bg-colorBrownBold' : 'bg-colorBrownSlightLV3'}`}>
                                      <Text className='font-bold text-colorWhite' >{item}</Text>
                                    </Pressable>
                          })
                        }
                      </View>
                    ...
                    <View className='mt-6'>
                      <Text className='mb-2 text-colorBrownDarkLV2 font-bold text-[18px]'>Từ Vựng</Text>
                      <View className='w-[300px] flex-row my-[5px] items-center justify-between 
                      rounded-md grid-cols-4 h-[50px] border-[1px] border-colorBrownDarkLV2'>
                        ...
                        placeholder='Tiếng Anh'></TextInput>
                        <TouchableOpacity 
                      ...
                      rounded-md grid-cols-4 h-[50px] border-[1px] border-colorBrownDarkLV2'>
                        ...
                        className='text-colorBrownDarkLV2 font-bold text-[17px] w-[54%] h-[80%]' 
                        placeholder='Tiếng Việt'></TextInput>
                        <TouchableOpacity 
                      ...
                      rounded-md grid-cols-4 h-[50px] '>
                        ...
                        className=' w-[94%] pl-[5px] text-[16px]' placeholder='Nhập ví dụ...'></TextInput>
                      </View>
                    </View>
                    <View className='w-[300px] grid-cols-2 flex-row justify-between '>
                        <View className='w-[170px] '>
                          <Text className='text-[20px] font-bold text-colorBrownDarkLV2'>{showEng && ` ${eng}`}</Text>
                          <Text className='ml-1 text-colorBrownSlightLV3'>{`${category && `(${category})`}`}</Text>
                          <Text className='text-[18px] mt-1 text-colorBrownDarkLV2 font-bold'>{showVie && ` ${vie}`}</Text>
                        </View>
