...
const LearnItemVocaOne = () => {
  ...
  const handleLearnVoca = async () => {
    if (listVoca?.length === 0) {
      ...
      indexCurrent.current =
        +indexCurrent.current < listVoca?.length - 1
          ? +indexCurrent.current + 1
          : 0;
      setVocabularyCurrent(listVoca[indexCurrent.current + 1]);
      // console.log("vocabularyCurrent?.progress : ", vocabularyCurrent);
    }
  };
  console.log("voca current", vocabularyCurrent?.progress);
  console.log(
    "calculator : ",
    +((+`${vocabularyCurrent?.progress}0` * 320) / 100) + 32
  );
  ...
  return (
    <View>
      <View className="w-screen relative h-screen  bg-red-400">
        ...
        >
          <View className="flex-col flex-1 px-screen items-center mt-12">
            <View className="flex justify-center items-center">
              <Text className="font-bold text-colorBorder text-[16px]">
                Từ chưa thuộc: {listVoca?.length}
              </Text>
            </View>
            <View className="flex-row justify-between mt-16">
              ...
              <View>
                <Text>{/* {vocabularyCurrent} */}</Text>
              </View>
              <View className="w-[150px] h-[200px] items-center flex justify-center">
                <Text
                  className={`${
                    vocabularyCurrent ? "text-[50px]" : " text-[25px]"
                  } 
                         text-colorBrownSlightLV3 font-bold`}
                >
                  {vocabularyCurrent
                    ? `${(+vocabularyCurrent?.progress / 10) * 10}0%`
                    : "Bạn đã hoàn thành bộ từ vựng này"}
                </Text>
              </View>
            </View>
            ...
            {/* <View
                className={`absolute w-[${20}px] h-5 rounded-xl bg-colorBrownSlightLV2`}
              ></View> */}
            <CircularProgress
              radius={60}
              value={+vocabularyCurrent?.progress * 10}
              valueSuffix={"%"}
              inActiveStrokeColor={"#765827"}
              inActiveStrokeOpacity={0.2}
              progressValueColor="#765827"
              activeStrokeColor="#765827"
            ></CircularProgress>
            {/* </View> */}
